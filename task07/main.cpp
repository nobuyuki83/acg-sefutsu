...
int main() {
  ...
  while (!glfwWindowShouldClose(viewer.window)) {
    ...
    Eigen::Vector3d pos_trg( 1.5+sin(3*time), cos(2*time), 1*cos(time)+1.5 ); // target position
    {
      ...
      const Eigen::Matrix<double,8,8> approximated_hessian = diff_pos_def.transpose() * diff_pos_def + alpha * Eigen::MatrixXd::Identity(8, 8);
      const Eigen::Matrix<double,8,1> delta_angle = -approximated_hessian.inverse() * diff_pos_def.transpose() * (pos_def - pos_trg);
